...
[![Schedule](https://img.shields.io/badge/⏰_Daily-08:17_%2B_08:47_Beijing-8b5cf6?style=for-the-badge)](https://github.com/DoTrungHuy/X-daily-digest/actions)
[![Cost](https://img.shields.io/badge/💰_X_API-Not_used-22c55e?style=for-the-badge)](https://dotrunghuy.github.io/X-daily-digest/)

**免费读 X（Cookie / twitter-cli）→ DeepSeek 完整精读 → GitHub Actions 双时点防漏 → GitHub Pages 日更。**
...
- **防漏日更**：北京时间 **08:17** 主任务、**08:47** 备用任务；当天已经生成就自动跳过，不重复抓取 X 或调用 DeepSeek
- **双线更新**：成功生成后既 commit 仓库，又主动部署 Pages
...
- **UTC 00:17 ≈ 北京/新加坡 08:17**：主任务
- **UTC 00:47 ≈ 北京/新加坡 08:47**：备用任务

备用任务开始后会先检查当天的 `digests/YYYY-MM-DD.md`。如果主任务已经成功生成，当天流水线会直接跳过，不再抓取 X、调用 DeepSeek、提交文件或重复部署 Pages；如果当天文件还不存在，备用任务才会执行完整流程。

我手动点击 **Run workflow** 时不受这个限制，即使当天文件已经存在，也可以主动重新生成和部署。
...
| 08:47 的备用任务为什么全部跳过 | 说明 08:17 已经成功生成当天摘要，这是正常的防重复保护，不会再次产生 DeepSeek 调用费用 |
| 想重新生成今天的摘要 | 在 Actions 中手动运行 `Daily X Digest`；手动运行会绕过当天文件检查 |
